...
export default function InvoiceForm({ invoice }: InvoiceFormProps) {
  ...
  useEffect(() => {
    // Generate a temporary client-side ID for display purposes for new invoices.
    // The real ID will be generated by Firestore.
    if (!isEditing) {
        setNewInvoiceId(`INV-${Date.now().toString().slice(-5)}`);
    }
